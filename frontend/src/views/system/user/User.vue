<template>
  <a-card :bordered="false" class="card-area">
    ...
    <div>
      <div class="operator">
        <a-button type="primary" ghost @click="add" v-hasPermission="'admin:user:add'">新增</a-button>
        <a-button @click="batchDelete" v-hasPermission="'admin:user:delete'">删除</a-button>
        <a-dropdown v-hasAnyPermission="'admin:user:reset','admin:user:export'">
          <a-menu slot="overlay">
            <a-menu-item v-hasPermission="'admin:user:reset'" key="password-reset" @click="resetPassword">密码重置</a-menu-item>
            <a-menu-item v-hasPermission="'admin:user:export'" key="export-data" @click="exportExcel">导出Excel</a-menu-item>
          </a-menu>
          <a-button>
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          <a-icon v-hasPermission="'admin:user:update'" type="setting" theme="twoTone" twoToneColor="#4a9ff5" @click="edit(record)" title="修改用户"></a-icon>
          &nbsp;
          <a-icon v-hasPermission="'admin:user:view'" type="eye" theme="twoTone" twoToneColor="#42b983" @click="view(record)" title="查看"></a-icon>
          <a-badge v-hasNoPermission="'admin:user:update','admin:user:view'" status="warning" text="无权限"></a-badge>
        </template>
      </a-table>
    </div>
    <!-- 用户信息查看 -->
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('admin/user/' + userIds.join(',')).then(() => {
    ...
    resetPassword () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$put('admin/user/password/reset', {
