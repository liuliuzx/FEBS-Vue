...
import { treeList, fetchList, delDictItem, delObj } from '@/api/sys/dict'
export default {
  name: 'DictList',
  ...
  methods: {
    loadData (arg) {
      ...
      this.loading = true
      // 查询字典
      treeList({dictName: this.queryParam.dictName, delFlag: this.queryParam.delFlag}).then((response) => {
    ...
    delDict () {
      ...
      if (!this.dict.id) {
        ...
        that.$confirm({
          ...
          onOk: function () {
            delObj(that.dict.id).then((response) => {
              const res = response.data
              if (res.code === 1) {
